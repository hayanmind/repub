/**
 * Card news renderer v3 — uses Gemini-generated background imagery.
 *
 * For each slide: pick a background from scripts/plock/assets/bg/{catDir}/
 * overlay dark gradient, then render layout on top.
 */
...
// v3 render timestamp
const V3_RENDERED_AT = new Date('2026-04-22T02:00:00Z').getTime();
...
// Pick background image for a given slide
// Priority: (1) slide-specific image bg/slides/{catDir}/{setName}/{slideIdx}.jpg
//           (2) category-level image bg/{catDir}/{n}.jpg
function backgroundFor(set: CardSet, slide: Slide): string {
  ...
  const slidePath = join(
    ...
    catDir,
    set.setName,
    `${String(slide.idx).padStart(2, '0')}.jpg`,
  );
  if (existsSync(slidePath)) return slidePath;
  // 2) category-level fallback (rotate deterministically)
...
function renderSlideHtml(slide: Slide, set: CardSet, format: Format, totalSlides: number): string {
  ...
  const bgUrl = bgPath ? imgToDataUrl(bgPath) : '';
  ...
  // Split layouts direction depends on template
...
/* Color-wash to tint image with category accent */
.overlay-tint {
...
.title .accent {
  color: ${p.accent};
  text-shadow: 0 0 40px ${p.accent}88;
}
.title .hl-bar {
...
.quote-mark {
  ...
  font-weight: 700;
  margin-bottom: -20px;
  text-shadow: 0 0 60px ${p.accent}88;
}
...
.flow-node-active {
  ...
  box-shadow: 0 16px 60px ${p.accent}aa;
  border: none;
}
...
.flow-line { flex: 0 0 ${sq ? 24 : 36}px; height: 2px; background: linear-gradient(90deg, ${p.accent}, ${p.accent}22); }
...
.list-num { font-size: ${sq ? 28 : 32}px; font-weight: 800; color: ${p.accent}; font-variant-numeric: tabular-nums; min-width: ${sq ? 52 : 60}px; }
...
.compare-after {
  background: linear-gradient(135deg, ${p.accent}44, ${p.accentDeep}22);
  border: 2px solid ${p.accent};
  box-shadow: 0 20px 80px ${p.accent}44;
}
.compare-tag {
  display: inline-block; padding: 6px 16px;
  background: rgba(255,255,255,0.15); color: ${p.inkSoft};
  border-radius: 999px;
  font-size: ${sq ? 14 : 16}px; font-weight: 800; letter-spacing: 0.15em;
  margin-bottom: 18px;
}
...
.compare-card li { font-size: ${sq ? 18 : 20}px; color: ${p.inkSoft}; padding-left: 20px; position: relative; }
.compare-card li::before { content: '•'; position: absolute; left: 0; color: ${p.accent}; font-weight: 800; }
.compare-arrow { display: flex; align-items: center; justify-content: center; filter: drop-shadow(0 0 12px ${p.accent}); }
...
.ig-icon {
  width: ${sq ? 64 : 80}px; height: ${sq ? 64 : 80}px;
...
.cta-btn {
  ...
  font-weight: 800;
  border-radius: 999px;
  box-shadow: 0 20px 80px ${p.accent}88;
}
.cta-btn .cta-arrow { display: flex; align-items: center; }
.cta-url { margin-top: 16px; font-size: ${sq ? 20 : 22}px; color: ${p.inkSoft}; letter-spacing: 0.08em; text-shadow: 0 2px 8px rgba(0,0,0,0.6); }
...
async function renderOne(
  ...
  if (existsSync(outPath)) {
    ...
    if (mtime >= V3_RENDERED_AT) return 'skipped';
  }
  const size = FORMAT_SIZE[format];
...
async function main() {
  const onlySet = process.env.ONLY_SET;
  ...
  if (onlySet) {
    targetSets = data.sets.filter(
      (s) => `${String(s.categoryIdx).padStart(2, '0')}-${s.category.replace(/\s+/g, '')}/${s.setName}` === onlySet,
    );
      ...
      for (const format of formats) {
        ...
        if (done % 10 === 0 || done === total) {
          ...
          process.stdout.write(
            `\r  ${done}/${total}  rendered=${rendered} skipped=${skipped}  ${rate.toFixed(1)}/s  ETA ${Math.round(eta)}s     `,
          );
        }
      }
    }
